;(function(root, factory) {
    ...
    var createTabPane = function (opts,isclose) {
        ...
        var tits = $.isFunction(opts.title) ? opts.title() : opts.title||"";
        var cons = $.isFunction(opts.content) ? opts.content() : opts.content||"";
        headCell.append(lis.html(tits).append(close));
        conCell.append(divs.html(cons));
        //判断是否可以关闭Tabs
        if(isclose){
        ...
        if ($.isFunction(opts.afterfun)) opts.afterfun();
    }
    //统一选项卡关闭
    ...
    jefn.init = function () {
        ...
        if ($.isFunction(opts.success)) opts.success();
